...
class MathParser:
    ...
    def __init__(self, num_type=float, num_base=10):
    ...
    def parse_expression(self, expression):
        ...
        while pointer < len(expression):
            ...
            if expression[pointer] in MathParser.NUMBERS:

                # If we meet number we start to read the number to the end
                number = ""
                while expression[pointer] in MathParser.NUMBERS or\
                        expression[pointer] == '.':
                ...
                if self.num_type == int:
                    out.append(int(number, self.num_base))
                else:
                    out.append(float(number))

                continue

            if expression[pointer] in MathParser.OPERATORS:
            ...
            if function_name in MathParser.FUNCTIONS:
    ...
    def shunting_yard(self, expression):
        ...
        for token in expression:
            if isinstance(token, self.num_type):
                out.append(token)
                continue

            if token in MathParser.FUNCTIONS:
            ...
            if token in MathParser.OPERATORS:
                op1 = MathParser.OPERATORS[token]
                if len(stack) == 0:
                    stack.append(token)
                    continue

                op2 = stack[-1]
                if op2 not in MathParser.OPERATORS:
                    stack.append(token)
                    continue

                op2 = MathParser.OPERATORS[op2]
            ...
            if token == ")":
                ...
                if stack[-1] in MathParser.FUNCTIONS:

                    if MathParser.FUNCTIONS[stack[-1]].arg_count == "multiple":
    ...
    def calculate(self, expression):
        ...
        if isinstance(expression, str):
            expression = self.shunting_yard(expression)
        ...
        for token in expression:
            ...
            if token in MathParser.OPERATORS:
                op2 = stack.pop()
                op1 = stack.pop()
                stack.append(MathParser.OPERATORS[token].func(op1, op2))
                continue
            ...
            if token in MathParser.FUNCTIONS:
                if MathParser.FUNCTIONS[token].arg_count == "multiple":
                    count = stack.pop()
                    args = [stack.pop() for _ in range(count)]
                    stack.append(MathParser.FUNCTIONS[token].func(args))
                else:
                    stack.append(MathParser.FUNCTIONS[token].func(stack.pop()))
                continue
...
if __name__ == "__main__":
    ...
    parsed = parser.parse_expression(inp)
    print(parsed)
    polish = parser.shunting_yard(parsed)
    print(polish)
    result = parser.calculate(polish)
    print(result)
